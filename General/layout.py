...
containerImageTooltip = """Executable or Container to run the replication. in the case of a 
container, it must be a Singularity image that contains the 
runtime used (Stata, R, Python, Julia) to run the analysis."""
...
dataTooltip = """Paths for data files. If the path(s) is(are) under the main directory, 
the entire folder, as well as sub-folders and files are copied 
...
containerImageFrameLayout = [
    [sg.Text('[3] Select Executable/Container Image')],
    [
...
dataFrameLayout = [
    ...
    [
        ...
        sg.Button('Browse', key='dataBrowse', tooltip='Ctrl+5', size=(8, 1)),
        sg.Button('Remove Selected', key='removeDataFolders', tooltip='Alt+5', size=(14, 1))
    ]    
]
# Dependencies layout
dependenciesFrameLayout = [
    [sg.Text('[6] Select Dependencies (Optional)')],
    [
        sg.Listbox(
            [],
            key='dependencies', 
        ...
        sg.Button('Browse', key='dependenciesBrowse', tooltip='Ctrl+6', size=(8, 1)),
        sg.Button('Remove Selected', key='removeDependencies', tooltip='Alt+6', size=(14, 1))
    ]    
]
# Tools
toolsFrameLayout = [
    [sg.Text('[7] Select Tools Folders (Optional)')],
    [
        sg.Listbox(
            [],
            key='tools', 
        ...
        sg.Button('Browse', key='toolsBrowse', tooltip='Ctrl+7', size=(8, 1)),
        sg.Button('Remove Selected', key='removeToolsFolders', tooltip='Alt+7', size=(14, 1))
    ]    
]
